# Kinematic input shaper to minimize motion vibrations in XY and Z planes
#
...
class InputShaperParams:
    def __init__(self, axis, config):
        ...
        raw_type = config.get('shaper_type_' + axis, shaper_type)
        self.shaper_type = str(raw_type).strip().lower()
        if self.shaper_type not in self.shapers:
            self.shaper_type = 'mzv'
...
class AxisInputShaper:
    ...
    def set_shaper_kinematics(self, sk):
        ...
        return ffi_lib.input_shaper_set_shaper_params(
...
class InputShaper:
    def __init__(self, config):
        ...
        gcode.register_command("SET_INPUT_SHAPER",
                               ...
                               desc="Set cartesian parameters for input shaper")
        gcode.register_command("UPDATE_INPUT_SHAPER",
                               self.cmd_UPDATE_INPUT_SHAPER,
                               desc="Update input shaper parameters")
    ...
    def _update_input_shaping(self, error=None):
        ...
        if failed:
            raise (error or self.printer.command_error)(
                "Failed to configure shaper(s) %s" % 
